setupCLONEID = function(host = 'localhost', port = '3306', user = NA, password = NA, database="CLONEID", schemaScript = "CLONEID_schema.sql", compress = FALSE)
{
    ...
    if (is.na(compress)) { compress = yml$mysqlConnection$compress }
    else { yml$mysqlConnection$compress = compress }
