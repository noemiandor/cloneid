...
# File paths
# master_branch_file <- "~/Downloads/master_SNU-668_genomeprofile.txt"
# test_branch_file <- "~/Downloads/test_tommy_SNU-668_genomeprofile.txt"
...
## Timing
- Comparison process took `r comparison_time$toc - comparison_time$tic` seconds.
"

# # Write report to an R Markdown file
# report_path <- "~/Downloads/genome_profile_comparison_report.Rmd"
# writeLines(report_content, con = report_path)
#
# # Render the report to a PDF
# rmarkdown::render(report_path, output_format = "pdf_document", output_file = "comparison_report.pdf")
#
...
# library(rmarkdown)
# 
# cls <- c("NUGC-4","NCI-N87","HGC-27","KATOIII","SNU-668" ,"MKN-45","SNU-638","SNU-601")
# 
# # Base file paths
# base_path <- "~/Downloads/"
# 
...
#   timing_components <- list()
#   
#   # Measure load time for master_branch
#   tic("Load master_branch")
#   master_branch <- read.table(master_branch_file)
#   load_master_time <- toc(quiet = TRUE)
#   timing_components$load_master_time_sec <- load_master_time$toc - load_master_time$tic
#   
#   # Measure load time for test_branch
#   tic("Load test_branch")
#   test_branch <- read.table(test_branch_file)
#   load_test_time <- toc(quiet = TRUE)
#   timing_components$load_test_time_sec <- load_test_time$toc - load_test_time$tic
#   
#   # Dimensions
#   dim_master_branch <- dim(master_branch)
#   dim_test_branch <- dim(test_branch)
...
#   num_cells_master <- dim_master_branch[2]
#   num_cells_test <- dim_test_branch[2]
#   
#   # Time per cell in seconds
#   timing_components$time_per_cell_master_sec <- timing_components$load_master_time_sec / num_cells_master
#   timing_components$time_per_cell_test_sec <- timing_components$load_test_time_sec / num_cells_test
#   
#   # Use column names from master_branch to check against test_branch
#   reference_columns <- names(master_branch)
#   target_columns <- names(test_branch)
...
#   num_total_columns_master_branch <- length(reference_columns)
#   
#   # Measure comparison processing time
#   tic("Comparison Processing")
#   
#   # Summary statistics for matching columns
#   matching_column_names <- reference_columns[matching_columns]
#   if (num_matching_columns > 0) {
...
#   }
#   
#   # End comparison processing timing
#   comparison_time <- toc(quiet = TRUE)
#   timing_components$comparison_time_sec <- comparison_time$toc - comparison_time$tic
#   
#   # Calculate total processing time for this cell line
#   total_processing_time_sec <- timing_components$load_master_time_sec +
#     timing_components$load_test_time_sec +
#     timing_components$comparison_time_sec
#   
#   # Generate the PDF report
#   report_content <- "
# # Comparison Report
...
# 
# ## Timing
# - Load time for master_branch: `r round(timing_components$load_master_time_sec, 5)` seconds.
# - Load time for test_branch: `r round(timing_components$load_test_time_sec, 5)` seconds.
# - Time per cell for master_branch: `r round(timing_components$time_per_cell_master_sec, 5)` seconds.
# - Time per cell for test_branch: `r round(timing_components$time_per_cell_test_sec, 5)` seconds.
# - Comparison processing time: `r round(timing_components$comparison_time_sec, 5)` seconds.
# - **Total processing time**: `r round(total_processing_time_sec, 5)` seconds.
# "
#   
#   # Create a report file name based on the cell line
#   report_filename <- paste0("genome_profile_comparison_report_", cell_line, ".Rmd")
