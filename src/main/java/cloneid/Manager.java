...
public final class Manager {
	...
	private static Map<String, Clone> gatherForDisplay(String selstmt, String cloneID_or_sampleName, Perspectives which)
			throws Exception {
	...
	public static Map<String, Profile> profiles(String sampleName, Perspectives which, boolean includeRoot)
			throws Exception {
		String selstmt = "SELECT size,cloneID from " + which.getTableName() + " where hasChildren=true AND origin=\'"
				+ sampleName + "\' AND whichPerspective=\'" + which + "\' ORDER BY size DESC;";
	...
	public static Map<String, Profile> profiles(int cloneID, Perspectives which, boolean includeRoot) throws Exception {
		String selstmt = "SELECT size,cloneID from " + which.getTableName() + " where hasChildren=true AND cloneID="
				+ cloneID + " AND whichPerspective=\'" + which + "\' ORDER BY size DESC;";
		return (gatherProfiles(selstmt, which, includeRoot));
	}
	...
	private static Map<String, Profile> gatherProfiles(String selstmt, Perspectives which, boolean includeRoot)
			throws Exception {
		CLONEID cloneid = new CLONEID();
		cloneid.connect();
	...
	public static Map<String, Clone> display(int cloneID, Perspectives which) throws Exception {
		String selstmt = "SELECT parent,cloneID,size from " + which.getTableName() + " where cloneID=" + cloneID
				+ " AND whichPerspective=\'" + which + "\' ORDER BY size DESC;";
	...
	public static Map<String, Clone> display(String sampleName, Perspectives which) throws Exception {
		String selstmt = "SELECT cloneID,size from " + which.getTableName()
				+ " where parent IS NULL AND hasChildren=true AND origin=\'" + sampleName + "\' AND whichPerspective=\'" + which
				+ "\' ORDER BY size DESC;";
		Map<String, Clone> cloneSizes = gatherForDisplay(selstmt, sampleName, which);
		return (cloneSizes);
	}

	// public static double[][] compare(int cloneID1, Perspectives which1, int cloneID2, Perspectives which2)
	// 		throws Exception {
	...
	// 		Helper.toDouble(c1.getProfile().getValues()),
	// 		Helper.toDouble(c2.getProfile().getValues())
	// 	};
	...
	// 	final String NO_CHANGES_MADE_TO_DB = "No changes made to database: "
	...
	// try {
	// Map<String, Clone> X = Manager.display("SNU-668",
	// Perspectives.GenomePerspective);
	// for (Clone clone : X.values()) {
	// System.out.println(clone.toString());
	// String Y = Manager.extractID(clone.toString());
	// Map<String, Profile> Z = Manager.profiles(parseInt(Y),
	// Perspectives.GenomePerspective, false);
